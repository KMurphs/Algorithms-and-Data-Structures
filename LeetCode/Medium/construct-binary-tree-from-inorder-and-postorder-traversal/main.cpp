...
public:
    ...
    TreeNode* buildTreeRecur(vector<int>& poArr, vector<int>& ioArr, int* poIdx, int* ioIdx, int rootVal) {
        ...
        if(*ioIdx >= ioArr.size() || ioArr[ioArr.size() - 1 - *ioIdx] == rootVal) return NULL;
        
        TreeNode* curr = new TreeNode(poArr[poArr.size() - 1 - (*poIdx)++]);
        curr->right  = buildTreeRecur(poArr, ioArr, poIdx, ioIdx, curr->val);
        (*ioIdx)++;
        curr->left = buildTreeRecur(poArr, ioArr, poIdx, ioIdx, rootVal);
        return curr;
    }
};
